...
const MembersTab = () => {
  ...
  const SHEET_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQl2vnGXi5SFo3-1dEta9w7YHbODcAsh9pPC6EPlBfjiScaVeMj3r2SXktMHUU_vbfYAh00pmTZio3n/pub?gid=746403294&single=true&output=csv";
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (csvText.trimStart().startsWith("<")) {
        setSyncResult({ error: "Could not fetch sheet CSV. Make sure the sheet is published to web as CSV." });
        setSyncing(false);
        return;
      }
      ...
      setSyncResult(res.error ? { error: res.error } : res);
      if (!res.error) load();
    } catch (e) {
      ...
      {syncResult && (
        <div className={`px-4 py-3 rounded-xl text-sm font-semibold border ${
          ...
          {syncResult.error
            ...
            : `Sync complete — ${syncResult.created} created, ${syncResult.updated} updated, ${syncResult.skipped} skipped (${syncResult.total} total)${syncResult.errors?.length ? ` | Errors: ${syncResult.errors.join(" | ")}` : ""}`}
        </div>
      )}
