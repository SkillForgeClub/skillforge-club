...
const Navbar = React.memo(() => {
  ...
  const user = getUser();
  const profilePath = user?.role === "admin" ? "/admin-portal" 
                    : user?.role === "mentor" ? "/mentor-portal" 
                    : "/student-portal";
  ...
  return (
    <nav className="fixed w-full z-50 bg-slate-950/5 backdrop-blur-xl border-b border-white/5 top-0">
        ...
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <div className="hidden md:flex justify-end w-[200px]">
            {loggedIn ? (
              <Link
                to={profilePath}
                className="px-6 py-2 rounded-full text-base font-bold bg-gradient-to-r from-purple-500 to-indigo-500 text-white hover:opacity-90 hover:scale-105 active:scale-95 transition-all duration-200 shadow-[0_0_20px_rgba(139,92,246,0.25)]"
              >
                Profile
              </Link>
            ) : (
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              onClick={closeMenu}
              className="fixed inset-0 top-16 z-40 bg-slate-950/60 backdrop-blur-sm md:hidden"
            />
            ...
            <motion.div
              ...
              className="md:hidden absolute top-16 left-0 w-full bg-slate-950/95 backdrop-blur-xl border-b border-white/5 z-50 max-h-[calc(100vh-4rem)] overflow-y-auto"
            >
                ...
                <div className="pt-4 border-t border-white/5">
                  {loggedIn ? (
                    <Link
                      to={profilePath}
                      onClick={closeMenu}
                      className="block w-full py-3.5 rounded-xl text-center text-lg font-bold bg-gradient-to-r from-purple-500 to-indigo-500 text-white hover:opacity-90 transition-opacity duration-200"
                    >
                      Profile
                    </Link>
                  ) : (
